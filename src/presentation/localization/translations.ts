export const english = {
  'common.cancel': 'Cancel',
  'common.confirm': 'Confirm',
  'common.close': 'Close',
  'common.delete': 'Delete',
  'common.done': 'Done',
  'common.retry': 'Try again',
  'common.edit': 'Edit',
  'common.save': 'Save',
  'common.settings': 'Settings',
  'common.back': 'Back',
  'common.moreOptions': 'More options',
  'common.loading': 'Loading',
  'common.choose': 'Choose',
  'common.previousYear': 'Previous year',
  'common.nextYear': 'Next year',
  'common.dateFormat': 'Date (YYYY-MM-DD)',
  'databaseError.title': 'Jarling could not start',
  'databaseError.description':
    'The local database could not be initialized. The connection was closed safely. Try again or restart the application.',
  'tabs.budget': 'Budget',
  'tabs.accounts': 'Accounts',
  'tabs.transactions': 'Transactions',
  'tabs.reports': 'Reports',
  'defaults.bills': 'Bills',
  'defaults.needs': 'Needs',
  'defaults.subscriptions': 'Subscriptions',
  'defaults.wants': 'Wants',
  'defaults.creditCardPayments': 'Credit Card Payments',
  'defaults.rentMortgage': '🏠 Rent/Mortgage',
  'defaults.phoneInternet': '📱 Phone & Internet',
  'defaults.utilities': '⚡ Utilities',
  'defaults.groceries': '🛒 Groceries',
  'defaults.transportation': '🚗 Transportation',
  'settings.title': 'Settings',
  'settings.development': 'DEVELOPMENT',
  'settings.sampleData': 'Sample data',
  'settings.sampleDescription':
    'Add an account with €2,000, allocations, two expenses and four targets using the default categories. Your data is not deleted and the sample is never duplicated.',
  'settings.populate': 'Populate with sample data',
  'settings.populating': 'Adding…',
  'settings.sampleAdded': 'Sample data added',
  'settings.sampleExists': 'Sample data already exists',
  'settings.sampleAddedDescription':
    'An account, four targets, allocations and two expenses were created using the default categories.',
  'settings.sampleExistsDescription': 'The sample set was not duplicated.',
  'settings.sampleError': 'Sample data could not be added',
  'settings.viewBudget': 'View Budget',
  'settings.budgetSection': 'BUDGET',
  'settings.appSection': 'APP',
  'settings.budgetName': 'Budget name',
  'settings.currency': 'Currency',
  'settings.numberFormat': 'Number format',
  'settings.currencyPlacement': 'Currency placement',
  'settings.dateFormat': 'Date format',
  'settings.displayOptions': 'Display options',
  'settings.theme': 'Theme',
  'settings.themeLight': 'Light theme',
  'settings.themeDark': 'Dark theme',
  'settings.themeSystem': 'Match system',
  'settings.systemFormat': 'Match system',
  'settings.decimalComma': '1.234,56',
  'settings.decimalPoint': '1,234.56',
  'settings.placementBefore': 'Before amount',
  'settings.placementAfter': 'After amount',
  'settings.lock': 'App lock',
  'settings.lockDescription':
    'Require the device credentials whenever you return to Jarling.',
  'settings.lockUnavailable': 'Device lock unavailable',
  'settings.lockUnavailableBody':
    'Configure a PIN, pattern, fingerprint or face unlock on the device first.',
  'settings.lockPrompt': 'Unlock Jarling',
  'settings.unlock': 'Unlock',
  'settings.locked': 'Jarling is locked',
  'settings.dataSection': 'DATA & PRIVACY',
  'settings.dataPortability': 'Data and backups',
  'settings.portabilityDescription':
    'Your plan stays in the private app storage. Portable backups can be protected with a password known only to you.',
  'settings.export': 'Export data',
  'settings.exporting': 'Exporting…',
  'settings.exportFormat': 'Readable JSON',
  'settings.exportWarningTitle': 'Export unencrypted data?',
  'settings.exportWarning':
    'The JSON export is readable by any app or person with access to the file. Store and share it carefully.',
  'settings.backup': 'Encrypted backup',
  'settings.createBackup': 'Create backup',
  'settings.backupCreated': 'Encrypted backup created',
  'settings.progress.encrypting': 'Encrypting backup…',
  'settings.progress.decrypting': 'Decrypting backup…',
  'settings.progress.reading': 'Reading backup…',
  'settings.progress.counting': 'Counting every last cent…',
  'settings.progress.balancing': 'Balancing the budget…',
  'settings.progress.sofa': 'Checking under the sofa…',
  'settings.progress.behave': 'Making numbers behave…',
  'settings.progress.evidence': 'Hiding the evidence…',
  'settings.progress.vault': 'Unlocking the vault…',
  'settings.progress.categories': 'Putting categories back…',
  'settings.progress.payments': 'Reordering payments…',
  'settings.progress.checkingFile': 'Checking every page…',
  'settings.restore': 'Restore backup',
  'settings.restoreWarningTitle': 'Replace the current plan?',
  'settings.restoreWarning':
    'Restoring a backup permanently replaces the current accounts, transactions, categories and targets.',
  'settings.restoreComplete': 'Backup restored',
  'settings.restoreCompleteBody':
    'Jarling has replaced the current plan with the backup.',
  'settings.encrypted': 'Password protected',
  'settings.password': 'Backup password',
  'settings.confirmPassword': 'Confirm password',
  'settings.passwordHelp':
    'Use at least 8 characters. Jarling cannot recover a forgotten backup password.',
  'settings.passwordLength': 'Use at least 8 characters.',
  'settings.passwordMismatch': 'The passwords do not match.',
  'settings.backupError':
    'The operation failed. Check the password and selected backup.',
  'settings.decryptError':
    "Couldn't decrypt this backup. Check your password and try again.",
  'settings.corruptBackup': 'This backup appears to be damaged or incomplete.',
  'settings.unsupportedFormat': 'This file is not a Jarling backup.',
  'settings.unsupportedVersion':
    'This backup was created with an unsupported version of Jarling.',
  'settings.invalidSnapshot':
    'This backup contains invalid or inconsistent plan data.',
  'settings.migrationError':
    'Jarling could not update this older backup format.',
  'settings.restoreError': 'Jarling could not restore this backup.',
  'settings.aboutSection': 'ABOUT',
  'settings.about': 'About',
  'settings.projectRepository': 'Project repository',
  'settings.aboutError': 'The project page could not be opened.',
  'settings.dataError': 'The data could not be exported.',
  'settings.saved': 'Settings saved',
  'settings.saveError': 'Settings could not be saved',
  'settings.deletePlan': 'Delete Plan',
  'settings.deletePlanTitle': 'Delete this plan?',
  'settings.deletePlanBody':
    'All accounts, transactions, assignments, categories and targets will be permanently deleted.',
  'settings.deletePlanConfirm': 'Delete Plan',
  'settings.planDeleted': 'Plan deleted',
  'settings.planDeletedBody':
    'A new empty plan with the default categories is ready.',
  'settings.deletePlanError': 'The plan could not be deleted.',
  'budget.title': 'Budget',
  'budget.chooseMonth': 'Choose budget month',
  'budget.edit': 'Edit Budget',
  'budget.readyToAssign': 'Ready to Assign',
  'budget.assignedTooMuch': 'Assigned too much',
  'budget.futureAssignmentsAvailable': 'Available from future assignments',
  'budget.usingFutureAssignments': 'Using {{amount}} from future assignments',
  'budget.newTransactionCount': '{{count}} new transaction',
  'budget.newTransactionsCount': '{{count}} new transactions',
  'budget.review': 'Review',
  'budget.createFirstGroup': 'Create your first group',
  'budget.createFirstGroupHint': 'For example: Needs, Wants or Savings.',
  'budget.createGroup': 'Create group',
  'budget.createCategory': 'Create Category',
  'budget.addTransaction': 'Transaction',
  'budget.newGroup': 'New group',
  'budget.newCategory': 'New category',
  'budget.renameGroup': 'Rename group',
  'budget.renameCategory': 'Rename category',
  'budget.groupName': 'Group name',
  'budget.categoryName': 'Category name',
  'budget.assigned': 'Assigned',
  'budget.available': 'Available',
  'budget.noVisibleCategories': 'No visible categories',
  'budget.overspent': 'Overspent {{spent}} of {{funded}}',
  'budget.spent': 'Spent {{spent}} of {{funded}}',
  'budget.funded': 'Funded {{funded}} of {{goal}}',
  'budget.moreNeeded': '{{amount}} more needed this month',
  'budget.activity': 'Activity',
  'budget.moveMoney': 'Move Money',
  'budget.details': 'Details',
  'budget.assignToReachTarget': 'Assign {{amount}} to reach target',
  'budget.assignToCoverOverspending': 'Assign {{amount}} to cover overspending',
  'budget.target': 'Target',
  'budget.tapToRename': 'Tap to rename',
  'budget.hide': 'Hide',
  'budget.unhide': 'Unhide',
  'budget.editTarget': 'Edit Target',
  'budget.addTarget': 'Add Target',
  'budget.noTarget': 'No target yet',
  'budget.costToBeMe': 'Cost to Be Me',
  'budget.monthTargets': '{{month}} Targets',
  'budget.hidden': 'Hidden',
  'budget.addCategoryTo': 'Add category to {{group}}',
  'budget.from': 'From',
  'budget.to': 'To',
  'budget.moveValidation':
    'Choose two different categories and a positive amount.',
  'budget.swap': 'Swap source and destination',
  'budget.swapInsufficient':
    'The selected destination does not have enough money to become the source.',
  'budget.selectSource': 'Choose source',
  'budget.selectDestination': 'Choose destination',
  'categoryDetails.title': 'Category details',
  'categoryDetails.balance': 'Balance',
  'categoryDetails.target': 'Target',
  'categoryDetails.question': 'How much do you need for {{name}}?',
  'categoryDetails.targetIntro':
    "When you create a target, we'll let you know how much money to set aside to stay on track over time.",
  'categoryDetails.createTarget': 'Create Target',
  'categoryDetails.assignMore':
    'Assign {{amount}} more this month to stay on track.',
  'categoryDetails.onTrack': 'You are on track to meet your target!',
  'categoryDetails.assign': 'Assign',
  'categoryDetails.totalToAssignBy': 'Total to assign by {{date}}',
  'categoryDetails.assignedSoFar': 'Assigned so far',
  'categoryDetails.toGo': 'To go',
  'categoryDetails.editTarget': 'Edit Target',
  'categoryDetails.notes': 'Notes',
  'categoryDetails.notesPlaceholder':
    'Add context, reminders or anything useful about this category…',
  'categoryDetails.saveNotes': 'Save Notes',
  'categoryDetails.notesSaved': 'Notes saved.',
  'categoryDetails.every': 'Every {{date}}',
  'categoryDetails.by': 'By {{date}}',
  'categoryDetails.ongoing': 'Ongoing',
  'categoryDetails.availableFromPrevious': 'Available from previous month',
  'categoryDetails.assignedForMonth': 'Assigned for {{month}}',
  'categoryDetails.activityInMonth': 'Activity in {{month}}',
  'categoryDetails.insufficientFunds':
    'Requested {{requested}} · Available {{available}} · Missing {{missing}}',
  'categoryDetails.insufficientFundsTitle': 'Not enough money to assign',
  'categoryDetails.insufficientFundsBody':
    'Ready to Assign cannot cover this allocation. Move {{missing}} from another category or reduce the target allocation.',
  'categoryDetails.deleteCategory': 'Delete Category',
  'categoryDetails.deleteTitle': 'Delete “{{name}}”?',
  'categoryDetails.deleteEmptyBody':
    'This category has no transactions or money assigned to it. This action cannot be undone.',
  'categoryDetails.deleteMoneyBody':
    'This category has money assigned to it. If you delete it, all assigned and available money will be moved back to Ready to Assign.',
  'categoryDetails.deleteReassignBody':
    'Before you can delete this category, you need to reassign its past activity. All {{count}} transactions, assigned amount ({{assigned}}), and available amount ({{available}}) will be reassigned to the category you choose.',
  'categoryDetails.selectCategory': 'Select Category',
  'categories.selectDestination': 'Select Category',
  'categories.createNamed': 'Create “{{name}}”',
  'categories.searchOrCreate': 'Search or create a category',
  'categories.noMatches': 'No matching categories',
  'categories.createTitle': 'Create Category',
  'categories.chooseGroup': 'Choose Category Group',
  'categories.namePlaceholder': 'For example, Restaurants',
  'categories.noGroups': 'Create a category group first',
  'reports.title': 'Reports',
  'reports.interval': 'Interval',
  'reports.period': 'Period',
  'reports.lastIntervals': 'Last {{count}}',
  'reports.interval.day': 'Day',
  'reports.interval.week': 'Week',
  'reports.interval.month': 'Month',
  'reports.interval.year': 'Year',
  'reports.unit.day': 'day',
  'reports.unit.week': 'week',
  'reports.unit.month': 'month',
  'reports.unit.year': 'year',
  'reports.spending': 'Spending',
  'reports.income': 'Income',
  'reports.netWorth': 'Net worth',
  'reports.total': 'Total',
  'reports.breakdownBy.day': 'Breakdown by day',
  'reports.breakdownBy.week': 'Breakdown by week',
  'reports.breakdownBy.month': 'Breakdown by month',
  'reports.breakdownBy.year': 'Breakdown by year',
  'reports.spendingComposition': 'Spending composition by category',
  'reports.minimum': 'Min. {{amount}} · {{interval}}',
  'reports.maximum': 'Max. {{amount}} · {{interval}}',
  'reports.spendingBreakdown': 'Breakdown by category',
  'reports.emptySpending': 'Categorised spending will appear here.',
  'reports.netIncome': 'NET INCOME',
  'reports.incomeMinusSpending': 'Income minus spending',
  'reports.incomeVsSpending': 'Income vs spending',
  'reports.netWorthUpper': 'NET WORTH',
  'reports.assetsMinusDebt': 'Assets minus debt',
  'reports.netWorthTrend': 'Net worth trend',
  'reports.assets': 'Assets',
  'reports.debt': 'Debt',
  'accounts.title': 'Accounts',
  'accounts.add': 'Add account',
  'accounts.reconcile': 'Reconcile',
  'accounts.totalOnBudget': 'TOTAL ON-BUDGET',
  'accounts.empty': 'No accounts yet',
  'accounts.emptyHint': 'Add the money you already have to start budgeting it.',
  'accounts.createFirst': 'Create first account',
  'accounts.close': 'Close account',
  'accounts.closeDescription': 'The complete history of {{name}} will be kept.',
  'accounts.reconcileDescription':
    'Compare Jarling with the confirmed bank balance.',
  'accounts.closeHistoryDescription': 'Keep its complete transaction history.',
  'accounts.closed': 'CLOSED',
  'accounts.options': 'Options ›',
  'accounts.checking': 'Checking',
  'accounts.savings': 'Savings',
  'accounts.cash': 'Cash',
  'accounts.creditCard': 'Credit card',
  'accounts.lineOfCredit': 'Line of credit',
  'accounts.cashAccounts': 'Cash Accounts',
  'accounts.cashAccountsDescription':
    'A cash account holds funds you already own and can spend immediately.',
  'accounts.creditAccounts': 'Credit Accounts',
  'accounts.creditAccountsDescription':
    'A credit account lets you spend borrowed money that you will need to repay later, often with interest.',
  'accounts.trackingAccounts': 'Tracking Accounts',
  'accounts.trackingAccountsDescription':
    'Track assets and debts without including them in your budget.',
  'accounts.tracking': 'Tracking',
  'accounts.loan': 'Loan',
  'accounts.onBudget': 'On-budget',
  'accounts.new': 'New account',
  'accounts.name': 'Name',
  'accounts.namePlaceholder': 'E.g. Main account',
  'accounts.type': 'Account type',
  'accounts.openingBalance': 'Opening balance',
  'accounts.openingBalanceHelp':
    'Enter the amount from cents. Use ± for negative balances.',
  'accounts.includeBudget': 'Include in the budget',
  'accounts.trackingHelp':
    'Tracking accounts do not participate in Ready to Assign.',
  'accounts.create': 'Create account',
  'accounts.creating': 'Creating…',
  'accounts.nameRequired': 'Enter an account name.',
  'accounts.createError': 'The account could not be created.',
  'accounts.budgetAccountDescription': 'Can be included in the budget.',
  'accounts.trackingDescription': 'Tracking only; not included in the budget.',
  'accounts.creditCardDescription':
    'Included in the budget with a linked payment category.',
  'accounts.loanDescription': 'Debt tracking; not included in the budget.',
  'accounts.details': 'Account details',
  'accounts.rename': 'Rename account',
  'accounts.workingBalance': 'Working balance',
  'accounts.clearedBalance': 'Cleared balance',
  'accounts.unclearedBalance': 'Uncleared balance',
  'accounts.balanceCounts':
    '{{cleared}} cleared · {{uncleared}} uncleared transactions',
  'reconciliation.title': 'Reconcile {{name}}',
  'reconciliation.question': 'What is the current bank balance?',
  'reconciliation.help':
    'Enter the confirmed balance, including only movements already reflected by the bank.',
  'reconciliation.clearedBalance': 'Cleared in Jarling',
  'reconciliation.workingBalance': 'Working balance',
  'reconciliation.difference': 'Difference',
  'reconciliation.counts': '{{cleared}} cleared · {{pending}} pending',
  'reconciliation.finish': 'Finish Reconciliation',
  'reconciliation.adjust': 'Create Adjustment & Reconcile',
  'reconciliation.success': 'Account reconciled',
  'reconciliation.successBody': 'Cleared transactions are now protected.',
  'reconciliation.adjustTitle': 'Create reconciliation adjustment?',
  'reconciliation.adjustBody':
    'Jarling will create a {{amount}} adjustment to match the confirmed balance.',
  'reconciliation.review': 'Review',
  'transactions.title': 'Transactions',
  'transactions.search': 'Search transactions',
  'transactions.refineSearch': 'Refine search',
  'transactions.suggestions': 'Suggestions',
  'transactions.searchingFor': 'Search transactions: “{{value}}”',
  'transactions.textFilter': 'Text',
  'transactions.add': 'Add transaction',
  'transactions.edit': 'Edit transaction',
  'transactions.expense': 'Expense',
  'transactions.openingBalance': 'Opening balance',
  'transactions.reconciliationAdjustment': 'Reconciliation adjustment',
  'transactions.income': 'Income',
  'transactions.transfer': 'Transfer',
  'transactions.new': 'New Transaction',
  'transactions.spending': 'Spending',
  'transactions.inflow': 'Inflow',
  'transactions.choosePayee': 'Choose Payee',
  'transactions.chooseCategory': 'Choose Category',
  'transactions.chooseAccount': 'Choose Account',
  'transactions.chooseDestinationAccount': 'Choose Destination Account',
  'transactions.chooseDestination': 'Choose Destination',
  'transactions.chooseDate': 'Choose Date',
  'transactions.fromAccount': 'From Account',
  'transactions.toAccount': 'To Account',
  'transactions.account': 'Account',
  'transactions.date': 'Date',
  'transactions.status': 'Status',
  'transactions.uncleared': 'Uncleared',
  'transactions.showMore': 'Show more',
  'transactions.showLess': 'Show less',
  'transactions.saving': 'Saving…',
  'transactions.amount': 'Amount',
  'transactions.type': 'Transaction type',
  'transactions.spendingDescription': 'Money leaving an account.',
  'transactions.inflowDescription': 'Money entering an account.',
  'transactions.transferDescription': 'Move money between two accounts.',
  'transactions.readyToAssign': 'Ready to Assign',
  'transactions.uncategorized': 'Uncategorized',
  'transactions.inflowRtaDescription': 'For new income.',
  'transactions.inflowCategoriesTitle': 'Categories',
  'transactions.inflowCategoryDescription':
    'For refunds, reimbursements, or other money returning to a category.',
  'transactions.memo': 'Memo',
  'transactions.addMemo': 'Add Memo',
  'transactions.memoTitle': 'Transaction Memo',
  'transactions.saveMemo': 'Save Memo',
  'transactions.reconciled': 'Reconciled',
  'transactions.cleared': 'Cleared',
  'transactions.pending': 'Pending',
  'transactions.deleteConfirmTitle': 'Delete transaction?',
  'transactions.deleteConfirmBody': 'This action cannot be undone.',
  'transactions.anything': 'Anything',
  'transactions.payee': 'Payee',
  'transactions.contains': '{{field}} contains: “{{value}}”',
  'transactions.noTransactions': 'No transactions yet',
  'transactions.noResults': 'No transactions match these filters.',
  'transactions.emptyHint': 'Record your first income or expense.',
  'transactions.accountsFilter': 'Accounts',
  'transactions.categoriesFilter': 'Categories',
  'transactions.clearedFilter': 'Cleared',
  'transactions.dateFilter': 'Date',
  'transactions.thisWeek': 'This week',
  'transactions.previousWeek': 'Previous week',
  'transactions.thisMonth': 'This month',
  'transactions.previousMonth': 'Previous month',
  'transactions.fromDate': 'From',
  'transactions.toDate': 'To',
  'transactions.fromDateValue': 'From: {{value}}',
  'transactions.toDateValue': 'To: {{value}}',
  'transactions.dateRangeFilter': 'Date: {{value}}',
  'transactions.selectAccountFilter': 'Account',
  'transactions.selectCategoryFilter': 'Category',
  'transactions.accountFilter': 'Account: {{value}}',
  'transactions.categoryFilter': 'Category: {{value}}',
  'transactions.statusFilter': 'Status: {{value}}',
  'transactions.clearSearch': 'Clear search',
  'transactions.deleteTransfer': 'Delete transfer?',
  'transactions.deleteTransferBody':
    'Both linked movements will be permanently deleted.',
  'transactions.reconciledTitle': 'Reconciled transaction',
  'transactions.reconciledBody': 'A reconciled transaction cannot be changed.',
  'transactions.amountRequired': 'Enter an amount greater than zero.',
  'transactions.accountRequired': 'Choose an account.',
  'transactions.destinationRequired': 'Choose a destination account.',
  'transactions.differentAccounts':
    'The source and destination accounts must be different.',
  'transactions.discardTitle': 'Discard this transaction?',
  'transactions.discardBody':
    'Your unsaved changes will be permanently discarded.',
  'transactions.continueEditing': 'Continue Editing',
  'transactions.discard': 'Discard',
  'payees.choose': 'Choose Payee',
  'payees.searchOrCreate': 'Search or create a payee',
  'payees.create': 'Create “{{name}}”',
  'payees.createHint': 'It will be available for future transactions.',
  'payees.empty': 'No payees yet',
  'payees.matching': 'MATCHING PAYEES',
  'payees.all': 'ALL PAYEES',
  'payees.emptyHint': 'No payees yet. Type a name to create the first one.',
  'targets.weekly': 'Weekly',
  'targets.monthly': 'Monthly',
  'targets.yearly': 'Yearly',
  'targets.custom': 'Custom',
  'targets.amount': 'Amount',
  'targets.iNeed': 'I need',
  'targets.every': 'Every',
  'targets.weeklyMonthlyModel':
    'Weekly occurrences calculate this month’s target. Your budget still rolls over monthly.',
  'targets.includePreviousWeeks': 'Include previous weeks',
  'targets.includePreviousWeeksDescription':
    'Also count occurrences earlier in this month. Future months always include the full month.',
  'targets.nextMonth': 'Next month I want to',
  'targets.by': 'By',
  'targets.lastDay': 'Last day',
  'targets.iWantTo': 'I want to',
  'targets.setAside': 'Set aside',
  'targets.fillUpTo': 'Fill up to',
  'targets.haveBalance': 'Have a balance of',
  'targets.setAsideAmount': 'Set aside another {{amount}}/{{period}}',
  'targets.refillAmount': 'Refill up to {{amount}}/{{period}}',
  'targets.setAsideDescription':
    'At the start of each month, leftover money does not reduce this month’s target.',
  'targets.refillDescription':
    'At monthly rollover, leftover available money reduces what you need to assign. There is no weekly reset.',
  'targets.customSetAsideDescription':
    'Use for bills and subscriptions. Add this amount regardless of the current balance.',
  'targets.customFillDescription':
    'Use for flexible spending. Refill the category as money is spent.',
  'targets.customBalanceDescription':
    'Use for savings over time. Build and maintain this available balance.',
  'targets.week': 'week',
  'targets.month': 'month',
  'targets.year': 'year',
  'targets.save': 'Save Target',
  'targets.set': 'Set Target',
  'targets.delete': 'Delete Target',
  'targets.deleteTitle': 'Delete target?',
  'targets.deleteDescription':
    'The target for {{name}} will be removed. Budget amounts will not change.',
  'targets.amountRequired': 'Enter an amount greater than zero.',
  'targets.deleteError': 'The target could not be deleted.',
  'targets.monthlyDay': 'Monthly target day',
  'targets.yearlyDate': 'Yearly target date',
  'targets.customDate': 'Target date',
  'targets.addDate': 'Add a target date',
  'targets.removeDate': 'Remove target date',
  'targets.snoozeThisMonth': 'Snooze for this month',
  'targets.unsnoozeThisMonth': 'Cancel target snooze',
  'targets.snoozedThisMonth': 'Target snoozed for this month',
  'form.enterName': 'Enter a name.',
  'form.couldNotSave': 'Could not save.',
  'form.saving': 'Saving…',
  'errors.invalidAccountName': 'Enter an account name.',
  'errors.accountBalanceNotZero':
    'Move or adjust the remaining balance before closing this account.',
  'errors.protectedTransaction':
    'Use the dedicated transfer or reconciliation workflow for this transaction.',
  'errors.invalidMoney': 'The balance can have at most two decimal places.',
  'errors.accountNotFound': 'The account no longer exists.',
  'errors.invalidCategoryName': 'Enter a category name.',
  'errors.invalidCategoryNotes':
    'Category notes cannot contain more than 4,000 characters.',
  'errors.invalidTarget': 'Check the target amount, frequency and date.',
  'errors.groupNotFound': 'The category group no longer exists.',
  'errors.categoryNotFound': 'The category no longer exists.',
  'errors.invalidAmount': 'Enter a positive amount greater than zero.',
  'errors.invalidDate': 'Enter a valid date in YYYY-MM-DD format.',
  'errors.invalidTransfer':
    'Choose two different accounts and a positive amount.',
  'errors.invalidReconciliation':
    'The confirmed balance does not match. Review the amount or authorize the adjustment.',
  'errors.trackingCategory':
    'Budget categories can only be used with accounts included in the budget.',
  'errors.categoryInflowAccount':
    'Category inflows are available only for accounts included in the budget.',
  'errors.closedAccount': 'Transactions cannot be added to a closed account.',
  'errors.transactionNotFound': 'The transaction no longer exists.',
  'errors.reconciledTransaction':
    'A reconciled transaction cannot be changed or deleted.',
  'errors.insufficientReadyToAssign':
    'There is not enough money in Ready to Assign for that allocation.',
  'errors.protectedCategory':
    'This category is managed by Jarling and cannot be changed.',
  'errors.categoryReassignmentRequired':
    'Choose another category for the existing transactions first.',
  'errors.invalidCategoryReassignment':
    'Choose a different valid destination category.',
  'errors.insufficientAvailable':
    'The source category does not have enough available money.',
  'errors.invalidBudgetMove':
    'Choose two different categories and a positive amount.',
  'errors.unknown': 'The operation could not be completed. Please try again.',
} as const;

export type TranslationKey = keyof typeof english;
export type TranslationDictionary = Readonly<Record<TranslationKey, string>>;

export const spanish: TranslationDictionary = {
  'common.cancel': 'Cancelar',
  'common.confirm': 'Confirmar',
  'common.close': 'Cerrar',
  'common.delete': 'Eliminar',
  'common.done': 'Hecho',
  'common.retry': 'Intentar de nuevo',
  'common.edit': 'Editar',
  'common.save': 'Guardar',
  'common.settings': 'Ajustes',
  'common.back': 'Volver',
  'common.moreOptions': 'Más opciones',
  'common.loading': 'Cargando',
  'common.choose': 'Elegir',
  'common.previousYear': 'Año anterior',
  'common.nextYear': 'Año siguiente',
  'common.dateFormat': 'Fecha (AAAA-MM-DD)',
  'databaseError.title': 'Jarling no ha podido iniciarse',
  'databaseError.description':
    'No se ha podido inicializar la base de datos local. La conexión se ha cerrado de forma segura. Inténtalo de nuevo o reinicia la aplicación.',
  'tabs.budget': 'Presupuesto',
  'tabs.accounts': 'Cuentas',
  'tabs.transactions': 'Transacciones',
  'tabs.reports': 'Informes',
  'defaults.bills': 'Facturas',
  'defaults.needs': 'Necesidades',
  'defaults.subscriptions': 'Suscripciones',
  'defaults.wants': 'Deseos',
  'defaults.creditCardPayments': 'Pagos de tarjetas de crédito',
  'defaults.rentMortgage': '🏠 Alquiler/Hipoteca',
  'defaults.phoneInternet': '📱 Teléfono e Internet',
  'defaults.utilities': '⚡ Suministros',
  'defaults.groceries': '🛒 Supermercado',
  'defaults.transportation': '🚗 Transporte',
  'settings.title': 'Ajustes',
  'settings.development': 'DESARROLLO',
  'settings.sampleData': 'Datos de ejemplo',
  'settings.sampleDescription':
    'Añade una cuenta con 2.000 €, asignaciones, dos gastos y cuatro objetivos usando las categorías predeterminadas. No elimina tus datos ni duplica el conjunto.',
  'settings.populate': 'Popular con datos de ejemplo',
  'settings.populating': 'Añadiendo…',
  'settings.sampleAdded': 'Datos de ejemplo añadidos',
  'settings.sampleExists': 'Los datos ya existen',
  'settings.sampleAddedDescription':
    'Se han creado una cuenta, cuatro objetivos, asignaciones y dos gastos usando las categorías predeterminadas.',
  'settings.sampleExistsDescription':
    'El conjunto de ejemplo no se ha duplicado.',
  'settings.sampleError': 'No se pudieron añadir los datos',
  'settings.viewBudget': 'Ver Presupuesto',
  'settings.budgetSection': 'PRESUPUESTO',
  'settings.appSection': 'APLICACIÓN',
  'settings.budgetName': 'Nombre del presupuesto',
  'settings.currency': 'Moneda',
  'settings.numberFormat': 'Formato numérico',
  'settings.currencyPlacement': 'Posición de la moneda',
  'settings.dateFormat': 'Formato de fecha',
  'settings.displayOptions': 'Opciones de visualización',
  'settings.theme': 'Tema',
  'settings.themeLight': 'Tema claro',
  'settings.themeDark': 'Tema oscuro',
  'settings.themeSystem': 'Igual que el sistema',
  'settings.systemFormat': 'Igual que el sistema',
  'settings.decimalComma': '1.234,56',
  'settings.decimalPoint': '1,234.56',
  'settings.placementBefore': 'Antes del importe',
  'settings.placementAfter': 'Después del importe',
  'settings.lock': 'Bloqueo de la aplicación',
  'settings.lockDescription':
    'Solicita las credenciales del dispositivo cada vez que vuelvas a Jarling.',
  'settings.lockUnavailable': 'Bloqueo no disponible',
  'settings.lockUnavailableBody':
    'Configura primero un PIN, patrón, huella o reconocimiento facial en el dispositivo.',
  'settings.lockPrompt': 'Desbloquear Jarling',
  'settings.unlock': 'Desbloquear',
  'settings.locked': 'Jarling está bloqueado',
  'settings.dataSection': 'DATOS Y PRIVACIDAD',
  'settings.dataPortability': 'Datos y copias',
  'settings.portabilityDescription':
    'Tu plan permanece en el almacenamiento privado de la aplicación. Las copias portátiles pueden protegerse con una contraseña que sólo tú conoces.',
  'settings.export': 'Exportar datos',
  'settings.exporting': 'Exportando…',
  'settings.exportFormat': 'JSON legible',
  'settings.exportWarningTitle': '¿Exportar datos sin cifrar?',
  'settings.exportWarning':
    'La exportación JSON puede leerla cualquier aplicación o persona con acceso al archivo. Guárdala y compártela con cuidado.',
  'settings.backup': 'Copia de seguridad cifrada',
  'settings.createBackup': 'Crear copia',
  'settings.backupCreated': 'Copia de seguridad cifrada creada',
  'settings.progress.encrypting': 'Cifrando la copia…',
  'settings.progress.decrypting': 'Descifrando la copia…',
  'settings.progress.reading': 'Leyendo la copia…',
  'settings.progress.counting': 'Contando hasta el último céntimo…',
  'settings.progress.balancing': 'Cuadrando el presupuesto…',
  'settings.progress.sofa': 'Mirando debajo del sofá…',
  'settings.progress.behave': 'Haciendo que los números se porten bien…',
  'settings.progress.evidence': 'Ocultando las pruebas…',
  'settings.progress.vault': 'Abriendo la caja fuerte…',
  'settings.progress.categories': 'Colocando las categorías en su sitio…',
  'settings.progress.payments': 'Reordenando los pagos…',
  'settings.progress.checkingFile': 'Revisando cada página…',
  'settings.restore': 'Restaurar copia',
  'settings.restoreWarningTitle': '¿Sustituir el presupuesto actual?',
  'settings.restoreWarning':
    'Restaurar una copia sustituye permanentemente las cuentas, transacciones, categorías y objetivos actuales.',
  'settings.restoreComplete': 'Copia restaurada',
  'settings.restoreCompleteBody':
    'Jarling ha sustituido el presupuesto actual por el de la copia.',
  'settings.encrypted': 'Protegida con contraseña',
  'settings.password': 'Contraseña de la copia',
  'settings.confirmPassword': 'Confirmar contraseña',
  'settings.passwordHelp':
    'Usa al menos 8 caracteres. Jarling no puede recuperar una contraseña olvidada.',
  'settings.passwordLength': 'Usa al menos 8 caracteres.',
  'settings.passwordMismatch': 'Las contraseñas no coinciden.',
  'settings.backupError':
    'La operación ha fallado. Comprueba la contraseña y la copia seleccionada.',
  'settings.decryptError':
    'No se pudo descifrar la copia. Comprueba la contraseña e inténtalo de nuevo.',
  'settings.corruptBackup': 'Esta copia parece estar dañada o incompleta.',
  'settings.unsupportedFormat': 'Este archivo no es una copia de Jarling.',
  'settings.unsupportedVersion':
    'Esta copia se creó con una versión de Jarling no compatible.',
  'settings.invalidSnapshot':
    'Esta copia contiene datos del presupuesto inválidos o incoherentes.',
  'settings.migrationError':
    'Jarling no pudo actualizar el formato antiguo de esta copia.',
  'settings.restoreError': 'Jarling no pudo restaurar esta copia.',
  'settings.aboutSection': 'ACERCA DE',
  'settings.about': 'Acerca de',
  'settings.projectRepository': 'Repositorio del proyecto',
  'settings.aboutError': 'No se pudo abrir la página del proyecto.',
  'settings.dataError': 'No se pudieron exportar los datos.',
  'settings.saved': 'Ajustes guardados',
  'settings.saveError': 'No se pudieron guardar los ajustes',
  'settings.deletePlan': 'Eliminar presupuesto',
  'settings.deletePlanTitle': '¿Eliminar este presupuesto?',
  'settings.deletePlanBody':
    'Se eliminarán definitivamente todas las cuentas, transacciones, asignaciones, categorías y objetivos.',
  'settings.deletePlanConfirm': 'Eliminar presupuesto',
  'settings.planDeleted': 'Presupuesto eliminado',
  'settings.planDeletedBody':
    'Ya puedes empezar con un presupuesto vacío y las categorías predeterminadas.',
  'settings.deletePlanError': 'No se pudo eliminar el presupuesto.',
  'budget.title': 'Presupuesto',
  'budget.chooseMonth': 'Elegir mes del presupuesto',
  'budget.edit': 'Editar presupuesto',
  'budget.readyToAssign': 'Disponible para asignar',
  'budget.assignedTooMuch': 'Asignado de más',
  'budget.futureAssignmentsAvailable': 'Disponible de asignaciones futuras',
  'budget.usingFutureAssignments': 'Usando {{amount}} de asignaciones futuras',
  'budget.newTransactionCount': '{{count}} transacción nueva',
  'budget.newTransactionsCount': '{{count}} transacciones nuevas',
  'budget.review': 'Revisar',
  'budget.createFirstGroup': 'Crea tu primer grupo',
  'budget.createFirstGroupHint': 'Por ejemplo: Necesidades, Deseos o Ahorro.',
  'budget.createGroup': 'Crear grupo',
  'budget.createCategory': 'Crear categoría',
  'budget.addTransaction': 'Transacción',
  'budget.newGroup': 'Nuevo grupo',
  'budget.newCategory': 'Nueva categoría',
  'budget.renameGroup': 'Renombrar grupo',
  'budget.renameCategory': 'Renombrar categoría',
  'budget.groupName': 'Nombre del grupo',
  'budget.categoryName': 'Nombre de la categoría',
  'budget.assigned': 'Asignado',
  'budget.available': 'Disponible',
  'budget.noVisibleCategories': 'No hay categorías visibles',
  'budget.overspent': 'Sobregastado: {{spent}} de {{funded}}',
  'budget.spent': 'Gastado: {{spent}} de {{funded}}',
  'budget.funded': 'Financiado: {{funded}} de {{goal}}',
  'budget.moreNeeded': 'Faltan {{amount}} este mes',
  'budget.activity': 'Actividad',
  'budget.moveMoney': 'Mover dinero',
  'budget.details': 'Detalles',
  'budget.assignToReachTarget': 'Asignar {{amount}} para cumplir el objetivo',
  'budget.assignToCoverOverspending':
    'Asignar {{amount}} para cubrir el gasto excesivo',
  'budget.target': 'Objetivo',
  'budget.tapToRename': 'Pulsa para renombrar',
  'budget.hide': 'Ocultar',
  'budget.unhide': 'Mostrar',
  'budget.editTarget': 'Editar objetivo',
  'budget.addTarget': 'Añadir objetivo',
  'budget.noTarget': 'Todavía no hay objetivo',
  'budget.costToBeMe': 'Coste de mi mes',
  'budget.monthTargets': 'Objetivos de {{month}}',
  'budget.hidden': 'Oculta',
  'budget.addCategoryTo': 'Añadir categoría a {{group}}',
  'budget.from': 'Desde',
  'budget.to': 'Hacia',
  'budget.moveValidation':
    'Elige dos categorías distintas y un importe positivo.',
  'budget.swap': 'Intercambiar origen y destino',
  'budget.swapInsufficient':
    'El destino seleccionado no tiene dinero suficiente para convertirse en origen.',
  'budget.selectSource': 'Elegir origen',
  'budget.selectDestination': 'Elegir destino',
  'categoryDetails.title': 'Detalles de la categoría',
  'categoryDetails.balance': 'Saldo',
  'categoryDetails.target': 'Objetivo',
  'categoryDetails.question': '¿Cuánto necesitas para {{name}}?',
  'categoryDetails.targetIntro':
    'Cuando crees un objetivo, te indicaremos cuánto dinero debes apartar para mantener el ritmo a lo largo del tiempo.',
  'categoryDetails.createTarget': 'Crear objetivo',
  'categoryDetails.assignMore':
    'Asigna {{amount}} más este mes para mantener el ritmo.',
  'categoryDetails.onTrack': '¡Vas al día para alcanzar tu objetivo!',
  'categoryDetails.assign': 'Asignar',
  'categoryDetails.totalToAssignBy': 'Total a asignar antes de {{date}}',
  'categoryDetails.assignedSoFar': 'Asignado hasta ahora',
  'categoryDetails.toGo': 'Pendiente',
  'categoryDetails.editTarget': 'Editar objetivo',
  'categoryDetails.notes': 'Notas',
  'categoryDetails.notesPlaceholder':
    'Añade contexto, recordatorios o cualquier dato útil sobre esta categoría…',
  'categoryDetails.saveNotes': 'Guardar notas',
  'categoryDetails.notesSaved': 'Notas guardadas.',
  'categoryDetails.every': 'Cada {{date}}',
  'categoryDetails.by': 'Antes de {{date}}',
  'categoryDetails.ongoing': 'Continuo',
  'categoryDetails.availableFromPrevious': 'Disponible del mes anterior',
  'categoryDetails.assignedForMonth': 'Asignado en {{month}}',
  'categoryDetails.activityInMonth': 'Actividad en {{month}}',
  'categoryDetails.insufficientFunds':
    'Solicitado {{requested}} · Disponible {{available}} · Faltan {{missing}}',
  'categoryDetails.insufficientFundsTitle':
    'No hay suficiente dinero para asignar',
  'categoryDetails.insufficientFundsBody':
    'Disponible para asignar no puede cubrir esta asignación. Mueve {{missing}} desde otra categoría o reduce la asignación del objetivo.',
  'categoryDetails.deleteCategory': 'Eliminar categoría',
  'categoryDetails.deleteTitle': '¿Eliminar “{{name}}”?',
  'categoryDetails.deleteEmptyBody':
    'Esta categoría no tiene transacciones ni dinero asignado. Esta acción no se puede deshacer.',
  'categoryDetails.deleteMoneyBody':
    'Esta categoría tiene dinero asignado. Si la eliminas, todo el dinero asignado y disponible volverá a Disponible para asignar.',
  'categoryDetails.deleteReassignBody':
    'Antes de eliminar esta categoría debes reasignar su actividad anterior. Sus {{count}} transacciones, el importe asignado ({{assigned}}) y el disponible ({{available}}) se reasignarán a la categoría que elijas.',
  'categoryDetails.selectCategory': 'Seleccionar categoría',
  'categories.selectDestination': 'Seleccionar categoría',
  'categories.createNamed': 'Crear “{{name}}”',
  'categories.searchOrCreate': 'Buscar o crear una categoría',
  'categories.noMatches': 'No hay categorías coincidentes',
  'categories.createTitle': 'Crear categoría',
  'categories.chooseGroup': 'Elegir grupo de categorías',
  'categories.namePlaceholder': 'Por ejemplo, Restaurantes',
  'categories.noGroups': 'Crea primero un grupo de categorías',
  'reports.title': 'Informes',
  'reports.interval': 'Intervalo',
  'reports.period': 'Periodo',
  'reports.lastIntervals': 'Últimos {{count}}',
  'reports.interval.day': 'Día',
  'reports.interval.week': 'Semana',
  'reports.interval.month': 'Mes',
  'reports.interval.year': 'Año',
  'reports.unit.day': 'día',
  'reports.unit.week': 'semana',
  'reports.unit.month': 'mes',
  'reports.unit.year': 'año',
  'reports.spending': 'Gastos',
  'reports.income': 'Ingresos',
  'reports.netWorth': 'Patrimonio',
  'reports.total': 'Total',
  'reports.breakdownBy.day': 'Desglose por día',
  'reports.breakdownBy.week': 'Desglose por semana',
  'reports.breakdownBy.month': 'Desglose por mes',
  'reports.breakdownBy.year': 'Desglose por año',
  'reports.spendingComposition': 'Composición del gasto por categoría',
  'reports.minimum': 'Mín. {{amount}} · {{interval}}',
  'reports.maximum': 'Máx. {{amount}} · {{interval}}',
  'reports.spendingBreakdown': 'Desglose por categorías',
  'reports.emptySpending': 'Los gastos categorizados aparecerán aquí.',
  'reports.netIncome': 'INGRESOS NETOS',
  'reports.incomeMinusSpending': 'Ingresos menos gastos',
  'reports.incomeVsSpending': 'Ingresos frente a gastos',
  'reports.netWorthUpper': 'PATRIMONIO NETO',
  'reports.assetsMinusDebt': 'Activos menos deuda',
  'reports.netWorthTrend': 'Evolución del patrimonio',
  'reports.assets': 'Activos',
  'reports.debt': 'Deuda',
  'accounts.title': 'Cuentas',
  'accounts.add': 'Añadir cuenta',
  'accounts.reconcile': 'Conciliar',
  'accounts.totalOnBudget': 'TOTAL EN PRESUPUESTO',
  'accounts.empty': 'Todavía no hay cuentas',
  'accounts.emptyHint':
    'Añade el dinero que ya tienes para empezar a presupuestarlo.',
  'accounts.createFirst': 'Crear primera cuenta',
  'accounts.close': 'Cerrar cuenta',
  'accounts.closeDescription': 'Se conservará todo el historial de {{name}}.',
  'accounts.reconcileDescription':
    'Compara Jarling con el saldo confirmado por el banco.',
  'accounts.closeHistoryDescription':
    'Conserva todo su historial de transacciones.',
  'accounts.closed': 'CERRADA',
  'accounts.options': 'Opciones ›',
  'accounts.checking': 'Corriente',
  'accounts.savings': 'Ahorro',
  'accounts.cash': 'Efectivo',
  'accounts.creditCard': 'Tarjeta de crédito',
  'accounts.lineOfCredit': 'Línea de crédito',
  'accounts.cashAccounts': 'Cuentas de efectivo',
  'accounts.cashAccountsDescription':
    'Una cuenta de efectivo contiene fondos que ya posees y puedes gastar inmediatamente.',
  'accounts.creditAccounts': 'Cuentas de crédito',
  'accounts.creditAccountsDescription':
    'Una cuenta de crédito permite gastar dinero prestado que tendrás que devolver, normalmente con intereses.',
  'accounts.trackingAccounts': 'Cuentas de seguimiento',
  'accounts.trackingAccountsDescription':
    'Controla activos y deudas sin incluirlos en tu presupuesto.',
  'accounts.tracking': 'Seguimiento',
  'accounts.loan': 'Préstamo',
  'accounts.onBudget': 'En presupuesto',
  'accounts.new': 'Nueva cuenta',
  'accounts.name': 'Nombre',
  'accounts.namePlaceholder': 'Ej. Cuenta principal',
  'accounts.type': 'Tipo de cuenta',
  'accounts.openingBalance': 'Saldo inicial',
  'accounts.openingBalanceHelp':
    'Introduce el importe desde los céntimos. Usa ± para saldos negativos.',
  'accounts.includeBudget': 'Incluir en el presupuesto',
  'accounts.trackingHelp':
    'Las cuentas de seguimiento no participan en Disponible para asignar.',
  'accounts.create': 'Crear cuenta',
  'accounts.creating': 'Creando…',
  'accounts.nameRequired': 'Introduce un nombre para la cuenta.',
  'accounts.createError': 'No se pudo crear la cuenta.',
  'accounts.budgetAccountDescription': 'Puede incluirse en el presupuesto.',
  'accounts.trackingDescription':
    'Sólo seguimiento; no participa en el presupuesto.',
  'accounts.creditCardDescription':
    'Se incluye en el presupuesto con una categoría de pago vinculada.',
  'accounts.loanDescription':
    'Seguimiento de deuda; no participa en el presupuesto.',
  'accounts.details': 'Detalles de la cuenta',
  'accounts.rename': 'Renombrar cuenta',
  'accounts.workingBalance': 'Saldo operativo',
  'accounts.clearedBalance': 'Saldo confirmado',
  'accounts.unclearedBalance': 'Saldo pendiente',
  'accounts.balanceCounts':
    '{{cleared}} confirmadas · {{uncleared}} transacciones pendientes',
  'reconciliation.title': 'Conciliar {{name}}',
  'reconciliation.question': '¿Cuál es el saldo actual del banco?',
  'reconciliation.help':
    'Introduce el saldo confirmado, incluyendo únicamente movimientos ya reflejados por el banco.',
  'reconciliation.clearedBalance': 'Confirmado en Jarling',
  'reconciliation.workingBalance': 'Saldo de trabajo',
  'reconciliation.difference': 'Diferencia',
  'reconciliation.counts': '{{cleared}} confirmadas · {{pending}} pendientes',
  'reconciliation.finish': 'Finalizar conciliación',
  'reconciliation.adjust': 'Crear ajuste y conciliar',
  'reconciliation.success': 'Cuenta conciliada',
  'reconciliation.successBody':
    'Las transacciones confirmadas han quedado protegidas.',
  'reconciliation.adjustTitle': '¿Crear ajuste de conciliación?',
  'reconciliation.adjustBody':
    'Jarling creará un ajuste de {{amount}} para igualar el saldo confirmado.',
  'reconciliation.review': 'Revisar',
  'transactions.title': 'Transacciones',
  'transactions.search': 'Buscar transacciones',
  'transactions.refineSearch': 'Refinar búsqueda',
  'transactions.suggestions': 'Sugerencias',
  'transactions.searchingFor': 'Buscar transacciones: “{{value}}”',
  'transactions.textFilter': 'Texto',
  'transactions.add': 'Añadir transacción',
  'transactions.edit': 'Editar transacción',
  'transactions.expense': 'Gasto',
  'transactions.openingBalance': 'Saldo inicial',
  'transactions.reconciliationAdjustment': 'Ajuste de conciliación',
  'transactions.income': 'Ingreso',
  'transactions.transfer': 'Transferencia',
  'transactions.new': 'Nueva transacción',
  'transactions.spending': 'Gasto',
  'transactions.inflow': 'Entrada',
  'transactions.choosePayee': 'Elegir beneficiario',
  'transactions.chooseCategory': 'Elegir categoría',
  'transactions.chooseAccount': 'Elegir cuenta',
  'transactions.chooseDestinationAccount': 'Elegir cuenta de destino',
  'transactions.chooseDestination': 'Elegir destino',
  'transactions.chooseDate': 'Elegir fecha',
  'transactions.fromAccount': 'Cuenta de origen',
  'transactions.toAccount': 'Cuenta de destino',
  'transactions.account': 'Cuenta',
  'transactions.date': 'Fecha',
  'transactions.status': 'Estado',
  'transactions.uncleared': 'No confirmada',
  'transactions.showMore': 'Mostrar más',
  'transactions.showLess': 'Mostrar menos',
  'transactions.saving': 'Guardando…',
  'transactions.amount': 'Importe',
  'transactions.type': 'Tipo de transacción',
  'transactions.spendingDescription': 'Dinero que sale de una cuenta.',
  'transactions.inflowDescription': 'Dinero que entra en una cuenta.',
  'transactions.transferDescription': 'Mover dinero entre dos cuentas.',
  'transactions.readyToAssign': 'Disponible para asignar',
  'transactions.uncategorized': 'Sin categoría',
  'transactions.inflowRtaDescription': 'Para ingresos nuevos.',
  'transactions.inflowCategoriesTitle': 'Categorías',
  'transactions.inflowCategoryDescription':
    'Para devoluciones, reembolsos u otro dinero que vuelve a una categoría.',
  'transactions.memo': 'Nota',
  'transactions.addMemo': 'Añadir nota',
  'transactions.memoTitle': 'Nota de la transacción',
  'transactions.saveMemo': 'Guardar nota',
  'transactions.reconciled': 'Conciliada',
  'transactions.cleared': 'Confirmada',
  'transactions.pending': 'Pendiente',
  'transactions.deleteConfirmTitle': '¿Eliminar transacción?',
  'transactions.deleteConfirmBody': 'Esta acción no se puede deshacer.',
  'transactions.anything': 'Cualquier campo',
  'transactions.payee': 'Beneficiario',
  'transactions.contains': '{{field}} contiene: “{{value}}”',
  'transactions.noTransactions': 'Todavía no hay transacciones',
  'transactions.noResults': 'No hay resultados para estos filtros.',
  'transactions.emptyHint': 'Registra tu primer ingreso o gasto.',
  'transactions.accountsFilter': 'Cuentas',
  'transactions.categoriesFilter': 'Categorías',
  'transactions.clearedFilter': 'Confirmación',
  'transactions.dateFilter': 'Fecha',
  'transactions.thisWeek': 'Esta semana',
  'transactions.previousWeek': 'Semana anterior',
  'transactions.thisMonth': 'Este mes',
  'transactions.previousMonth': 'Mes anterior',
  'transactions.fromDate': 'Desde',
  'transactions.toDate': 'Hasta',
  'transactions.fromDateValue': 'Desde: {{value}}',
  'transactions.toDateValue': 'Hasta: {{value}}',
  'transactions.dateRangeFilter': 'Fecha: {{value}}',
  'transactions.selectAccountFilter': 'Cuenta',
  'transactions.selectCategoryFilter': 'Categoría',
  'transactions.accountFilter': 'Cuenta: {{value}}',
  'transactions.categoryFilter': 'Categoría: {{value}}',
  'transactions.statusFilter': 'Estado: {{value}}',
  'transactions.clearSearch': 'Borrar búsqueda',
  'transactions.deleteTransfer': '¿Eliminar transferencia?',
  'transactions.deleteTransferBody':
    'Se eliminarán definitivamente los dos movimientos enlazados.',
  'transactions.reconciledTitle': 'Transacción conciliada',
  'transactions.reconciledBody':
    'No se puede modificar una transacción conciliada.',
  'transactions.amountRequired': 'Introduce un importe mayor que cero.',
  'transactions.accountRequired': 'Selecciona una cuenta.',
  'transactions.destinationRequired': 'Selecciona una cuenta de destino.',
  'transactions.differentAccounts':
    'Las cuentas de origen y destino deben ser diferentes.',
  'transactions.discardTitle': '¿Descartar esta transacción?',
  'transactions.discardBody':
    'Los cambios que no hayas guardado se perderán definitivamente.',
  'transactions.continueEditing': 'Continuar editando',
  'transactions.discard': 'Descartar',
  'payees.choose': 'Elegir beneficiario',
  'payees.searchOrCreate': 'Buscar o crear un beneficiario',
  'payees.create': 'Crear “{{name}}”',
  'payees.createHint': 'Estará disponible para futuras transacciones.',
  'payees.empty': 'Todavía no hay beneficiarios',
  'payees.matching': 'BENEFICIARIOS COINCIDENTES',
  'payees.all': 'TODOS LOS BENEFICIARIOS',
  'payees.emptyHint':
    'Todavía no hay beneficiarios. Escribe un nombre para crear el primero.',
  'targets.weekly': 'Semanal',
  'targets.monthly': 'Mensual',
  'targets.yearly': 'Anual',
  'targets.custom': 'Personalizado',
  'targets.amount': 'Importe',
  'targets.iNeed': 'Necesito',
  'targets.every': 'Cada',
  'targets.weeklyMonthlyModel':
    'Las ocurrencias semanales calculan el objetivo de este mes. El presupuesto sigue haciendo rollover mensual.',
  'targets.includePreviousWeeks': 'Incluir semanas anteriores',
  'targets.includePreviousWeeksDescription':
    'Cuenta también las ocurrencias anteriores de este mes. Los meses futuros siempre incluyen el mes completo.',
  'targets.nextMonth': 'El próximo mes quiero',
  'targets.by': 'Antes de',
  'targets.lastDay': 'Último día',
  'targets.iWantTo': 'Quiero',
  'targets.setAside': 'Apartar',
  'targets.fillUpTo': 'Rellenar hasta',
  'targets.haveBalance': 'Tener un saldo de',
  'targets.setAsideAmount': 'Apartar otros {{amount}}/{{period}}',
  'targets.refillAmount': 'Rellenar hasta {{amount}}/{{period}}',
  'targets.setAsideDescription':
    'Al comenzar cada mes, el dinero sobrante no reduce el objetivo de ese mes.',
  'targets.refillDescription':
    'En el rollover mensual, el dinero disponible sobrante reduce lo que debes asignar. No hay reinicio semanal.',
  'targets.customSetAsideDescription':
    'Úsalo para facturas y suscripciones. Añade este importe sin importar el saldo actual.',
  'targets.customFillDescription':
    'Úsalo para gasto flexible. Rellena la categoría a medida que se gasta.',
  'targets.customBalanceDescription':
    'Úsalo para ahorrar con el tiempo. Construye y mantiene este saldo disponible.',
  'targets.week': 'semana',
  'targets.month': 'mes',
  'targets.year': 'año',
  'targets.save': 'Guardar objetivo',
  'targets.set': 'Crear objetivo',
  'targets.delete': 'Eliminar objetivo',
  'targets.deleteTitle': '¿Eliminar objetivo?',
  'targets.deleteDescription':
    'Se eliminará el objetivo de {{name}}. Los importes del presupuesto no cambiarán.',
  'targets.amountRequired': 'Introduce un importe mayor que cero.',
  'targets.deleteError': 'No se pudo eliminar el objetivo.',
  'targets.monthlyDay': 'Día del objetivo mensual',
  'targets.yearlyDate': 'Fecha del objetivo anual',
  'targets.customDate': 'Fecha del objetivo',
  'targets.addDate': 'Añadir fecha objetivo',
  'targets.removeDate': 'Quitar fecha objetivo',
  'targets.snoozeThisMonth': 'Pausar durante este mes',
  'targets.unsnoozeThisMonth': 'Cancelar pausa del objetivo',
  'targets.snoozedThisMonth': 'Objetivo pausado durante este mes',
  'form.enterName': 'Introduce un nombre.',
  'form.couldNotSave': 'No se pudo guardar.',
  'form.saving': 'Guardando…',
  'errors.invalidAccountName': 'Introduce un nombre para la cuenta.',
  'errors.accountBalanceNotZero':
    'Mueve o ajusta el saldo restante antes de cerrar esta cuenta.',
  'errors.protectedTransaction':
    'Usa el flujo específico de transferencia o conciliación para esta transacción.',
  'errors.invalidMoney': 'El saldo debe tener como máximo dos decimales.',
  'errors.accountNotFound': 'La cuenta ya no existe.',
  'errors.invalidCategoryName': 'Introduce un nombre para la categoría.',
  'errors.invalidCategoryNotes':
    'Las notas de la categoría no pueden superar los 4.000 caracteres.',
  'errors.invalidTarget':
    'Revisa el importe, la frecuencia y la fecha del objetivo.',
  'errors.groupNotFound': 'El grupo de categorías ya no existe.',
  'errors.categoryNotFound': 'La categoría ya no existe.',
  'errors.invalidAmount': 'Introduce un importe positivo mayor que cero.',
  'errors.invalidDate': 'Introduce una fecha válida con formato AAAA-MM-DD.',
  'errors.invalidTransfer':
    'Elige dos cuentas distintas y un importe positivo.',
  'errors.invalidReconciliation':
    'El saldo confirmado no coincide. Revisa el importe o autoriza el ajuste.',
  'errors.trackingCategory':
    'Las categorías solo se pueden usar con cuentas incluidas en el presupuesto.',
  'errors.categoryInflowAccount':
    'Las entradas por categoría solo están disponibles en cuentas incluidas en el presupuesto.',
  'errors.closedAccount':
    'No se pueden añadir movimientos a una cuenta cerrada.',
  'errors.transactionNotFound': 'La transacción ya no existe.',
  'errors.reconciledTransaction':
    'Una transacción conciliada no se puede modificar ni eliminar.',
  'errors.insufficientReadyToAssign':
    'No hay suficiente dinero en Disponible para asignar para esa asignación.',
  'errors.protectedCategory':
    'Jarling gestiona esta categoría y no permite modificarla.',
  'errors.categoryReassignmentRequired':
    'Elige primero otra categoría para las transacciones existentes.',
  'errors.invalidCategoryReassignment':
    'Elige una categoría de destino válida y diferente.',
  'errors.insufficientAvailable':
    'La categoría de origen no tiene suficiente dinero disponible.',
  'errors.invalidBudgetMove':
    'Elige dos categorías distintas y un importe positivo.',
  'errors.unknown': 'No se pudo completar la operación. Inténtalo de nuevo.',
};
